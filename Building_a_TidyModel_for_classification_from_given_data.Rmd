...
pkgs <- c("broom", "broom.mixed","readr","skimr","readxl","remotes","glmnet","tidymodels","skimr","parallel","dplyr","magrittr","doParallel","vip","themis","plotly","ConfusionTableR","DT")
...
```{r load_in}
# Read in the data
setwd("C:/Users/l018917/OneDrive - Eli Lilly and Company/Desktop/virtue by success/tyi")
table11 <- read_excel("table11.xlsx")

datatable(table11,extensions = 'Buttons',
options = list(dom='Bfrtip',
buttons=c('copy', 'csv', 'excel', 'print', 'pdf')))
table112<-table11 %>% select("anb_angle","bjork_sum","overbite","max_ci_to_sn","max_ci_to_op","impa","mand_ci_to_op","interincisal_angle","ul_to_eline","ll_to_eline","nasolabial_angle","archlen_upper","archlen_lower","molar_key_score","profile","protrusion_index","chief_complaint_score","result_code") %>% drop_na()
...
class_bal_table <- table(table112$result_code)
...
```{r sep_data}
factors <- names(select_if(table112, is.factor))
numbers <- names(select_if(table112, is.numeric))
characters <- names(select_if(table112, is.character))
table112$result_code<-as.factor(table112$result_code)
...
set.seed(123)
split <- rsample::initial_split(table112, prop=3/4)
...
stranded_rec <- 
  recipe(result_code ~ ., data=train_data) %>% 
...
plot <- strand_fitted %>% 
  ...
  geom_col() + theme(axis.text.x = element_text(
                                        ...
                                        size=8, angle=90)
                                                         ...
                                                         caption="Produced by Significance")

#print("Creating plot of P values")
#print(plot)
plotly::ggplotly(plot)
#print(ggplotly(plot))
ggsave("plot.png") #Save the plot
...
roc_plot <- 
  stranded_preds %>% 
  roc_curve(truth = result_code, LR_NotStrandedProb) %>% 
  autoplot
...
```{r cm}
library(caret)
cm <- caret::confusionMatrix(stranded_preds$result_code,
...
cm <- ConfusionTableR::binary_visualiseR(
  train_labels = stranded_preds$result_code,
  truth_labels = stranded_preds$LR_Class,
  class_label1 = "No Surgery", 
  class_label2 = "Surgery",
...
rf_fit <- 
  rf_mod %>% 
  fit(result_code ~ ., data = train_data)
...
rf_wf <- 
  workflow() %>% 
  add_model(rf_mod) %>% 
  add_formula(result_code ~ .) #The predictor is contained in add_formula method
...
#   roc_curve(result_code, `.pred_Not Stranded`) %>% 
#   autoplot()
# 
# print(roc_plot)
# ggsave("roc_plot.png")
...
xgboost_score_train <- 
  train_prediction %>% 
  yardstick::metrics(result_code, .pred_class) 
...
cm_outputs <- ConfusionTableR::binary_class_cm(train_labels = test_prediction$.pred_class,
                                 truth_labels = test_prediction$result_code)
